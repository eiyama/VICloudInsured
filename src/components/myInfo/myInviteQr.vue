  ...
  <div class="home" style="background-color:RGB(9,52,131);min-height: inherit;">
    ...
    <div class="mint-content" style="margin-left:30px;margin-right:30px">
      <div class="tip-qr" style="background-color:#FFF;border-radius:8px;padding:30px">
        <div class="tip-title"><span>{{  weixinInfor.nickname ? weixinInfor.nickname : nickName}}的名片</span></div>
        <p v-show="scanCode" style="color:#d00000;">我的邀请码：{{userCode}}</p>
         <p v-show="!scanCode" style="color:#d00000; height: 40px;">我的邀请码：{{userCode}}</p>
        <div v-if="scanCode">
            <img :src="scanCode" style="height:200px" alt="">
        </div>
...
export default {
      ...
      axiosGetAPI(url).then((res) => {
        if (res.status == 200 && res.result && res.result.weixin) {
          ...
          if(this.appId == 'cx7190761141531rzi' || this.appId == 'cxa181211170855pxj'){
            ...
            shareImg = "https://bjzxbx.bandins.com/zxhome/img/zxlogo.png";
          }
  ...
  created() {
   if(sessionStorage.getItem("weixinInfor") ){
    ...
    this.nickName =  this.weixinInfor.nickname || this.$route.query.nickName;
