<template>
  <div style="padding-bottom:55px">
    ...
    <div class="hmint-content">
      ...
      <div style="border:1px solid #F5F5F5;background-color:#FFF;border-radius:8px;padding-bottom:15px;margin-left:10px;margin-right:10px;position: relative;top: -15px;">
        ...
        <van-row  type="flex" justify="end">
          ...
          <!-- <van-col span="4">
            <p @click="choiceCity" style="line-height:45px;float:right;color:#999">切换</p>
          </van-col> -->
          <!-- <van-col span="2">
            <img @click="choiceCity" :src="defaultImg" style="width:28px;height:28px;margin-top:8px;margin-left:8px">
          </van-col> -->
        </van-row>
        ...
        <div v-show="currentIndex == 0">
          <div style="position: relative;padding-left:10px;">
            ...
            <img v-show="canUseOcr" @click="showOccre" style="height: 30px;position: absolute;right: 10px;top: 10px;" src="../../assets/images/icon-Photo.png" />
          </div>
        ...
        <div v-show="currentIndex == 1">
           <div style="position: relative;padding-left:10px;">
              ...
              <img v-show="canUseOcr" @click="vinOccre" style="height: 30px;position: absolute;right: 10px;top: 10px;" src="../../assets/images/icon-Photo.png" />
          </div>
          <div style="position: relative;" >
        ...
        <div style="padding:0 10px;">
          ...
          <mt-button class="mt20 btn_quote" style="margin-top: 20px;color: #4a66fe;background: #fff;border:1px solid #4A66FE;" type="default" size="large" @click="toInquiry" v-show="hasEnquiryProducts">人工询价</mt-button>
        </div>  
      </div>
    </div>
    <div style="border:1px solid #F5F5F5;background-color:#FFF;border-radius:8px;padding-bottom:15px;
      padding-left:10px;padding-right:10px;margin-left:10px;margin-right:10px;" v-show="hasCustomerData || hasMoreCustomerData">
    ...
    <div class="carIncustomer" @click="carInCustomers" v-show="hasViewCars == 'true'">
  ...
  methods: {
   ...
   created () {
    ...
    if (appId) {
      ...
      self.hasEnquiryProducts = (appId == 'cx7190761141531rzi' || appId == 'cxa181211170855pxj' ||  appId == 'cx8190110143618pyi' || appId == 'cxs190103104249iwe' || appId == 'cx7190408201251bh0');
                    ...
                    // if(!openId){
                        axiosPostAPI('/Weixin/' + code + '/Token').then(dataRes=>{//微信免登陆，如果api返回了token则实现免登陆，否则去登陆。
                            ...
                            if(self.isZXapp){ //如果是中兴智能车则去zxregister.vue(中兴注册页面)；
                              self.$store.commit(types.LOGOUT);
                              self.$router.push({ path: "/myInfo/zxregister?appId=" + localStorage.getItem('appId') + '&showOrg=true'});
                           }else{
                              // Toast('登录失效，请重新登录');
                              self.$store.commit(types.LOGOUT);
                              self.$router.push({ path: "/login?appId=" + localStorage.getItem('appId')});
